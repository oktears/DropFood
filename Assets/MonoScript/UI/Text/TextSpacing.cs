...
namespace UnityEngine.UI
{
    ...
    public class TextSpacing : BaseMeshEffect
    {
        ...
        public enum HorizontalAligmentType { Left, Center, Right }
        public int lineTextCount = 10;
        ...
        public override void ModifyMesh(VertexHelper vh)
        {
            ...
            for (var i = 0; i < lines.Length; i++)
            {
                for (var j = lines[i].StartVertexIndex; j <= lines[i].EndVertexIndex; j++)
                {
                    ...
                    {
                        Vector3 offset = new Vector3(Spacing * ((j - lines[i].StartVertexIndex) / 6), 0, 0);
                        vt.position += offset;
                    }
                    ...
                    {
                        var offset = (charCount / 6) % 2 == 0 ? 0.5f : 0f; vt.position += new Vector3(Spacing * ((j - lines[i].StartVertexIndex) / 6 - charCount / 12 + offset), 0, 0);
                    }
                    vertexs[j] = vt;
                    // 以下注意点与索引的对应关系 
                    if (j % 6 <= 2)
