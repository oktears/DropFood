...
Shader "Chengzi/Water/Island2" {
Properties {
	[NoScaleOffset] _ControlTex ("控制图", 2D) = "white" {}
	[NoScaleOffset] _TerrainTex ("地形图", 2D) = "white" {}
...
half4 frag( v2f i ) : SV_Target
{
	...
	fixed4 color = tex2D(_TerrainTex, i.mainuv.xy);

	
	if (control.b > 0) 
	{
		fixed3 normal = UnpackNormal(tex2D(_BumpMap, i.mainuv.xy * 200));
