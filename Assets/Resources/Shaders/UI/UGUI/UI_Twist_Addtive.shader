...
Shader "Chengzi/UI/UGUI/Twist_Additive【热扭曲_高亮叠加】"
{
	Properties
	{
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			fixed4 frag(v2f IN) : SV_Target
			{
	         	...
	         	float twisted = tex2D(_TwistedTex, IN.texcoord + (_Time * _TwistingSpeed).x).x * _TwistingIntensity;
	         	
	         	float4 nn = tex2D(_OperateTex, IN.texcoord + twisted.xx);
