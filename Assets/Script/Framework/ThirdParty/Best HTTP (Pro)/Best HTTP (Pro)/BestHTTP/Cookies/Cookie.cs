...
namespace BestHTTP.Cookies
{
    ...
    public sealed class Cookie : IComparable<Cookie>, IEquatable<Cookie>
    {
        ...
        public Cookie(Uri uri, string name, string value, DateTime expires, bool isSession)
        ...
        public Cookie(Uri uri, string name, string value, long maxAge, bool isSession)
            : this(name, value, uri.AbsolutePath, uri.Host)
        {
            this.MaxAge = maxAge;
            this.IsSession = isSession;
        ...
        private static List<HeaderValue> ParseCookieHeader(string str)
        {
            ...
            {
                // Read key
                string key = str.Read(ref idx, (ch) => ch != '=' && ch != ';', true).Trim();
