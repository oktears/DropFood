        ...
        void IManager.OnPacket(Packet packet)
        {
            ...
            switch (packet.TransportEvent)
            {
                case TransportEventTypes.Ping:
                    (this as IManager).SendPacket(new Packet(TransportEventTypes.Pong, SocketIOEventTypes.Unknown, "/", string.Empty, 0, 0));
                    break;
        ...
        void IHeartbeat.OnHeartbeatUpdate(TimeSpan dif)
        {
            switch (State)
            {
                ...
                case States.Open:
                    ...
                    {
                        (this as IManager).SendPacket(new Packet(TransportEventTypes.Ping, SocketIOEventTypes.Unknown, "/", string.Empty, 0, 0));
