...
public class SchedulingDemo : MonoBehaviour
{
	...
	void Update()
	{
		...
		//renderer.enabled = (transform.localScale.x > 0.01f) ? true : false;

		// make the cube fade back to red if made yellow
		//renderer.material.color = Color.Lerp(renderer.material.color, Color.red, Time.deltaTime * 3f);
	...
	public void SmackCube()
	{
		...
		//rigidbody.maxAngularVelocity = 1000;	// enable high rotation velocities
		//rigidbody.AddTorque(rotation);

		// play the 'smack' sound
		//audio.PlayOneShot(m_SmackSound);
